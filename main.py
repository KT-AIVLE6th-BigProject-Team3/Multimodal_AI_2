...
device = torch.device('cpu') # GPU 설정 | GPU가 없으면 'cpu'로 바꿔주세요.
...
def load_AGV_model(Model_Parameter='Parameters/AGV_Best_State_Model.pth'):
    ...
    model.load_state_dict(torch.load(Model_Parameter, map_location=device))  # map_location을 명시적으로 cpu로 설정
    return model.to(device).eval()  # 모델을 CPU로 이동
...
# AGV | OHT 평가
oht_y_true, oht_y_pred = Evaluation_Classification_Model(
    OHTConditionClassifier, oht_test_dataloader, 'OHTClassification', device=device
)
agv_y_true, agv_y_pred = Evaluation_Classification_Model(
    AGVConditionClassifier, agv_test_dataloader, 'AGVClassification', device=device
)
